...
# ------------------------------------------------------------------
# Required Part A function tests
# ------------------------------------------------------------------
# This sidebar button tests the weather function with the two
# locations required by the lab without running the tests every
# time Streamlit reloads the page.

with st.sidebar.expander("Required function tests"):

    st.write(
        "Test get_current_weather with the two locations "
        "required in Part A."
    )

    if st.button("Test Syracuse and Lima"):
        ...
        try:
            ...
            st.json(lima_weather)

        except Exception as error:
            st.error(
                f"Weather test failed: {error}"
            )